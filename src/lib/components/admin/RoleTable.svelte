<script lang="ts">
	...
	const columns = [
		...
		},
		{
			...
			label: 'Permissions',
			sortValue: (role: Role) => role.permissions.length
		},
		{
...
<div class="table-container col-12">
	<SortableTable data={roles} {columns} rowKey={(role) => role.id} rowClass={() => 'role-row'}>
		{#snippet cellContent({ item: role, column })}
			...
			{:else if column.label === 'Permissions'}
